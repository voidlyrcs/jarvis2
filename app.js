...
recognition.onresult = function (event) {
  ...
  if(localStorage.getItem("lang") === "en-US"){
    if (transcript.includes("hi jarvis")) {
     readOut(`merhaba ${JSON.parse(userData).name}`);
    }
    ...
    if (transcript.includes("full weather report")) {
      readOut("hava raporunu açıyorum efendim");
      let a = window.open(
    ...
    if (transcript.includes("are you there")) {
      readOut("Evet efendim");
    }
    // close voice recognition
    if (transcript.includes("close")) {
      readOut("tamam hocam ben biraz uyuyacağım");
      stopingR = true;
      recognition.stop();
    }
  ...
  // spotify
    if (transcript.includes("open spotify")) {
      readOut("spotifyı açıyorum");
      let a = window.open("https://open.spotify.com/");
      windowsB.push(a)
    }
    if (transcript.includes("how are you")) {
      readOut(`iyiyim sen nasılsın${JSON.parse(userData).name}`);
    }
...
var adres="yüz.html";
var ayarlar = 'width=350,height=350,left=250,top=100,scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no'
 
window.open(adres, 'ornek', ayarlar);
    }
    ...
    if (transcript.includes("play")) {
      ...
      readOut(`youtube sonuçları ${videoName}`);
    ...
    if (transcript.includes("open my instagram profile")) {
      if (JSON.parse(userData).instagram) {
        readOut("instagram profilini açıyorum");
        let a =window.open(
